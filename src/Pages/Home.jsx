...
>
          ...
          <div className={`w-full md:w-[40vw] h-[100%] fixed top-0 right-0 bg-white shadow-x1 p-6 transition-all duration-500 flex flex-col items-center overflow-auto ${showCart?"translate-x-0":"translate-x-full"}`}>
            <header className="w-[100%] flex justify-between items-center">
            <span className="text-green-400 text-[18px] font-semibold">Order Items</span>
            <RxCross2 className="w-[30px]  h-[20px] text-green-400 text-[18px] font-semibold cursor-pointer hover:bg-gray-600" onClick={()=>setShowCart(false)}/>
            </header>
            {items.length>0?<>
        ...
        <button className="w-[80%] p-3 rounded-lg bg-green-400 text-gray-700 hover:bg-green-600 transition-all" onClick={()=>{
